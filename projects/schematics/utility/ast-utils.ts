 ...
 * or after the last of occurrence of `syntaxKind` if the last occurrence is a
 * sub child of ts.SyntaxKind[nodes[i].kind] and save the changes in file.
 *
 * @param nodes insert after the last occurrence of nodes
 * @param toInsert string to insert
 * @param file file to insert changes into
 * @param fallbackPos position to insert if toInsert happens to be the first
 * occurrence
  ...
  if (node.importClause) {
    if (node.importClause.name) {
      ...
      if (nb.kind == ts.SyntaxKind.NamespaceImport) {
        ...
        return { [(nb as ts.NamespaceImport).name.text + '.']: modulePath };
      } else {
...
export function getDecoratorMetadata(
  ...
  const angularImports: { [name: string]: string } = findNodes<ts.ImportDeclaration>(
    source,
  ...
  return getSourceNodes(source)
    ...
    .filter(expr => {
      if (expr.expression.kind == ts.SyntaxKind.Identifier) {
        ...
        // If the left expression is not an identifier, just give up at that
        // point.
...
/**
 * Given a source file with @NgModule class(es), find the name of the first
 * @NgModule class.
 *
...
export function getFirstNgModuleName(source: ts.SourceFile): string | undefined {
  ...
  // Then walk parent pointers up the AST, looking for the ClassDeclaration
  // parent of the NgModule metadata.
...
export function addSymbolToNgModuleMetadata(
  ...
  const matchingProperties: ts.ObjectLiteralElement[] = (node as ts.ObjectLiteralExpression).properties
    ...
    // Filter out every fields that's not "metadataField". Also handles
    // string literals (but not expressions).
    .filter(prop => {
...
/**
 * Custom function to insert an NgModule into NgModule imports. It also imports
 * the module.
 */
...
export function isImported(
  ...
  const matchingNodes = allNodes
    .filter((node): node is ts.ImportDeclaration => node.kind === ts.SyntaxKind.ImportDeclaration)
