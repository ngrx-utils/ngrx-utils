...
# Query Bazel for npm_package and ng_package rules with tags=["publish"]
# Publish them to npm (tagged next)

# query for all npm packages to be released as part of the framework release
NPM_PACKAGE_LABELS=`bazel query --output=label 'attr("tags", "\[.*publish.*\]", //projects/...) intersect kind(".*_package", //projects/...)'`
