...
task(
  'stylelint',
  execNodeTask('stylelint', [stylesGlob, '--config', 'stylelint-config.json', '--syntax', 'scss'])
);
...
task('ownerslint', () => {
  ...
  let ownedPaths = readFileSync(ownersFilePath, 'utf8')
    .split('\n')
    // Trim lines.
    .map(line => line.trim())
  ...
  let ignoredPaths = readFileSync(gitIgnorePath, 'utf8')
    .split('\n')
    // Trim lines.
    .map(line => line.trim())
    // Remove empty lines and comments.
