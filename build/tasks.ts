 ...
 */
export async function copyExcecutableFiles(config: Config) {
  const packages = util.getTopLevelPackages(config).filter(pkg => pkg === 'cli');

  const binaryFiles = await util.getListOfFiles(`./modules/?(${packages.join('|')})/bin/*`);
